...
__clock(초)      // 마감 시간 조작
__coin(n)        // 코인 지급
__giveAll()      // 의상·장비 전부 + 소모품 STOCK_CAP(5)개씩 — 상점/효과 테스트용
...
12. **지갑(`/wallet`)의 아이템 카탈로그를 Worker 에 복제하지 말 것.**
    ...
    **소모품 상한 `STOCK_CAP` 은 세 곳이 다 봐야 한다** — `SHOP.buy()`(구매 차단) ·
    재고 로더(localStorage) · `SHOP.apply()`(서버 지갑). 서버는 아이템을 모르니
    **상한도 클라이언트만 안다.** `apply()` 를 빼먹으면 상한을 올리기 전 지갑이
    서버에서 그대로 되돌아와 상한이 무의미해진다.
...
16. **문항을 SQL 로 고칠 때 개수(`PACK_N` = 20/10/12)를 바꾸지 말 것.**
    ...
    **문항 원본은 `leaderboard-worker/content-source.json` 하나다.**
    번들 배열과 SQL 을 손으로 따로 고치면 반드시 어긋난다. 원본에서 둘 다 만들고,
    게임 파일의 `PACKS` 를 **실행해 덤프한 값**과 SQL 결과를 대조해서 확인할 것.
