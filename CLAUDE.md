...
11. **지갑(`/wallet`)의 아이템 카탈로그를 Worker 에 복제하지 말 것.**
...
12. **🔑 기기 이동 코드는 `pid` 자체이고 사실상 비밀번호다.**
...
15. **순위표 탭의 서버 필터와 로컬 폴백 필터를 같이 고칠 것.**
